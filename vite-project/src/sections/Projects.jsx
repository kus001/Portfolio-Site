...
export const Projects = () => {
  return (
    <section id="projects" className="py-32 relative overflow-hidden -mt-20">
      {/* bg glow */}
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {projects.map((project, idx) => (
            <a 
              ...
              href={project.github}
              target="_blank"
              className="group glass rounded-2xl overflow-hidden animate-fade-in md:row-span-1 block transition-all"
            ...
            >
              ...
              <div className="p-6 space-y-3">
                <div className="flex items-start justify-between">
                  <h3 className="text-xl font-semibold group-hover:text-primary transition-colors font-code">
                    {project.title}
                  </h3>
                  ...
                  <div className="space-y-2 py-1">
                    ...
                    <div className="flex gap-2 w-full">
                      {[...Array(10)].map((_, blockIdx) => (
                        <div 
                          key={blockIdx} 
                          className={`flex-1 h-3 rounded-sm transition-all duration-500 animate-fade-in animation-delay-100 ${
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tags.map((tag, tagIdx) => (
                    <span key={tagIdx} className="px-3 py-1.5 rounded-full bg-surface text-xs font-medium font-code tracking-wider border border-border/50 text-muted-foreground hover:border-primary/50 hover:text-primary transition-all duration-300">
                      {tag}
                    </span>
                  ))}
                </div>
